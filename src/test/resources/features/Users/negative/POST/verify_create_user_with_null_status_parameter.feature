...
Feature: Verify GoREST API POST request to create new user with empty "status" parameter
  ...
  @create_user
  Scenario: Create new user empty status
    Given I create HTTP "POST" request to Users API with "users" endpoint
    And I set "{\"name\":\"Allasani Peddana\", \"email\":\"\", \"status\":null, \"gender\":\"male\"}" as request body
