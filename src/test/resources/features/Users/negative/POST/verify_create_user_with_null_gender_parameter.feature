...
Feature: Verify GoREST API POST request to create new user with empty "gender" parameter
  ...
  @create_user
  Scenario: Create new user with empty gender
    Given I create HTTP "POST" request to Users API with "users" endpoint
    And I set "{\"name\":\"Allasani Peddana\", \"email\":\"\", \"status\":\"active\", \"gender\":null}" as request body
