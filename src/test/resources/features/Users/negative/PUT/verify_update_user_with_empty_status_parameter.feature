...
Feature: Verify GoREST API PUT request to update user with empty "status" parameter
  ...
  @update_user
  Scenario: Update user with empty status
    ...
    And I set "{\"name\":\"Updated Name\", \"email\":\"\", \"status\":\"\"}" as request body
    And I set "email" as "userEmail" retrieved from stored data
    And I send the request
